...
def main():
    ...
    set_param("WP_CANCEL_AUX_CH", 4)
    set_param("WP_FLP_TRD_DIST", 0.5)    # front 0.3 ≤ 0.5 → flip_ready passes
    set_param("WP_TOP_CT_DIST", 0.05)    # injected body-Z 0.04 ≤ 0.05 → contact detected
    ...
    # Body -Z suction/contact sensor (orientation=24): required throughout and
    # used for wall contact after the 90-degree rotation.
    # Both are sent in one thread.
    print("[*] injecting front (0.3m) + body-Z contact (0.04m) distance sensors ...")
    def inject_thread():
        while running:
            # Front (orientation=0)
            m.mav.distance_sensor_send(0, 10, 500, 30, 0, 0, 0, 0)
            time.sleep(0.02)
            # Body -Z/upward contact sensor (id=1, orientation=24)
            m.mav.distance_sensor_send(0, 1, 500, 4, 0, 1, 24, 0)
            time.sleep(0.03)
    threading.Thread(target=inject_thread, daemon=True).start()
    time.sleep(0.5)  # let a few samples arrive

    # --- Trigger wall_perch on configured aux1 ---
    ...
    print("[*] done. The body-Z contact signal should now gate entry into WALL_PIN.")
    running = False
