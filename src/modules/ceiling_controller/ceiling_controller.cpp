...
//===================================================================
//  CeilingController — nav_and_controllers WQ
//
//  距离数据链路：
//  ESP32 -> TELEM2 UART -> uart_rx -> esp32_uart_frame -> ceiling_controller
//
//  注意：本模块不直接打开 /dev/ttyS4，也不直接读取 UART，避免和 uart_rx 抢占串口。
//===================================================================
...
bool CeilingController::init()
{
	...
	// 检查 uart_rx 发布的原始 ESP32 帧话题是否可用。开机顺序中 uart_rx 会先启动，
	// 但如果 ESP32 暂时还没发数据，这里只提示并继续等待。
	if (!_esp32_frame_sub.advertised()) {
		PX4_WARN("[CeilCtrl] esp32_uart_frame topic not advertised yet");
	} else {
		PX4_INFO("[CeilCtrl] esp32_uart_frame topic available");
	}
	
	PX4_INFO("[CeilCtrl] Started, subscribing to esp32_uart_frame from uart_rx");
...
bool CeilingController::validate_esp32_frame(const esp32_uart_frame_s &frame) const
{
...
float CeilingController::parse_ceiling_distance_m(const esp32_uart_frame_s &frame) const
{
	// ESP32 发送的是二进制大端距离：AA 55 high low reserved 0D 0A。
	// uart_rx 已经完成串口接收和基础帧同步，这里再做一次防御性校验后解析。
	const uint16_t distance_mm = (static_cast<uint16_t>(frame.frame[2]) << 8) | frame.frame[3];
	return static_cast<float>(distance_mm) * 0.001f;
}
...
void CeilingController::Run()
{
	...
	if (_first_run) { _first_run = false;
		mavlink_log_info(&_mavlink_log_pub, "[CeilCtrl] STARTED, subscribing to esp32_uart_frame"); }

	parameters_update(false);

	// 从 uart_rx 发布的 esp32_uart_frame 获取上视距离。
	// 这里的数据单位在 UART 帧里是 mm，控制器内部统一换算成 m。
	esp32_uart_frame_s uart_frame{};
	if (_esp32_frame_sub.update(&uart_frame)) {
		if (validate_esp32_frame(uart_frame)) {
			_ceiling_distance = parse_ceiling_distance_m(uart_frame);
			_last_distance_update = now;
			_uart_distance_active = true;
			...
			_uart_distance_active = false;
			PX4_WARN("[CeilCtrl] invalid esp32_uart_frame");
		}
	} else {
		// 一段时间没有新帧时，状态里标记数据源不活跃；保留最后距离值，避免状态机突然跳变。
		if (_last_distance_update == 0 || hrt_elapsed_time(&_last_distance_update) > UART_DISTANCE_TIMEOUT) {
			_uart_distance_active = false;
		}
	}
...
int CeilingController::print_status()
{
	PX4_INFO("running");
	PX4_INFO("state: %u", static_cast<unsigned>(_state));
	PX4_INFO("uart_rx distance source: %s", _uart_distance_active ? "active" : "inactive");
	PX4_INFO("ceiling distance: %.3f m", static_cast<double>(_ceiling_distance_lpf));
	PX4_INFO("compression: %.3f m", static_cast<double>(_compression));
	PX4_INFO("arm switch: %d, detach switch: %d", static_cast<int>(_ceiling_arm_switch), static_cast<int>(_detach_switch));
	PX4_INFO("fault count: %u", static_cast<unsigned>(_fault_count));

	if (_last_distance_update > 0) {
		PX4_INFO("last uart_rx frame: %llu ms ago",
			 static_cast<unsigned long long>((hrt_absolute_time() - _last_distance_update) / 1000));

	} else {
		PX4_INFO("last uart_rx frame: none");
	}
