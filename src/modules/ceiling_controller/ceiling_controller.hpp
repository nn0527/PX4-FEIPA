...
private:
	...
	bool validate_esp32_frame(const esp32_uart_frame_s &frame) const;
	float parse_ceiling_distance_m(const esp32_uart_frame_s &frame) const;
	...
	static constexpr uint64_t UART_DISTANCE_TIMEOUT{500_ms};
	hrt_abstime _attach_detect_start{0};
	hrt_abstime _dist_stable_start{0};
	hrt_abstime _last_distance_update{0};
	...
	uORB::Subscription _esp32_frame_sub{ORB_ID(esp32_uart_frame)};
	...
	// uart_rx 是否正在持续提供有效上视距离帧。
	bool _uart_distance_active{false};
